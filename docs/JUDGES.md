...
| 2:00 | Click **Exposed** → `checkout-api`. Path from `algo.MSpaths`. |
...
| 2:50 | Cypher drawer: `direct_lockfile_hits`, `package_releases`, `ms_paths`. |
...
**What we built:** HydraShield, an incident workspace over a HydraDB graph. Time is a 360-second replay plus a counterfactual yank. Completeness is three rings (org in-window, ecosystem reverse dependents, adjacent next-hop) plus the HAS_RELEASE that introduced the worm.

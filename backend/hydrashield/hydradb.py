...
class HydraDB:
    ...
    def merge_vertices(self, label: str, rows: list[dict[str, Any]], set_clause: str = "") -> int:
        ...
        cypher = f"UNWIND $rows AS row MERGE (n {{id: row.vertex}}) SET n:{label}, {set_clause}"
        try:
            ...
            for row in rows:
                ...
                self.execute(
                    f"MERGE (n {{id: $vertex}}) SET n:{label}, {assignments}",
    ...
    def merge_edges(self, rel: str, rows: list[dict[str, Any]]) -> int:
        ...
        try:
            ...
            for row in rows:
                self.execute(
                    f"MATCH (s {{id: $source}}), (d {{id: $destination}}) "
                    f"MERGE (s)-[r:{rel} {{id: $rel}}]->(d)",
                    row,
                )
        return len(rows)
